...
from flask import (
    ...
    jsonify, Response, copy_current_request_context, stream_with_context,
)
...
if not OPENAI_API_KEY:
    log.critical("OPENAI_API_KEY missing"); sys.exit(1)
...
TOOLS = [
    {
        ...
        "parameters": {
            ...
            "required": ["location"],        # unit is optional now
...
def _finish_tool(response_id: str, run_id: str | None, tool_call_id: str,
                 name: str, args_json: str, q: queue.Queue[Any]) -> str:
    output = _run_tool(name, args_json)
    follow = client.responses.create(
        model=MODEL,
        input=[{"type": "function_call_output",
                "call_id": tool_call_id, "output": output}],
        previous_response_id=response_id, tools=TOOLS, stream=True,
    )
    return _pipe(follow, q, run_id)


def _pipe(stream, q: queue.Queue[Any], run_id: str | None = None) -> str:
    response_id, resp_pushed = "", False
    early: list[str] = []                   # buffer tokens before meta sent
    ...
    for ev in stream:
        ...
        # first envelope with Response
        if getattr(ev, "response", None):
            response_id = ev.response.id
            if not resp_pushed:
                q.put({"meta": {"prev_id": response_id}})
                for t in early: q.put(t)
                early.clear(); resp_pushed = True

        # assistant text chunk
        if isinstance(delta, str) and "arguments" not in typ:
            (q.put if resp_pushed else early.append)(delta)

        # ── tool call handling (old + new schemas) ────────────
        if getattr(ev, "tool_calls", None):
            for tc in ev.tool_calls:
                ...
                if full_args:
                    response_id = _finish_tool(response_id, run_id,
                                               call_id, fn_name, full_args, q)

        if typ == "response.output_item.added":
            item = (getattr(ev, "item", None) or
                    getattr(ev, "output_item", None) or
                    ev.model_dump(exclude_none=True).get("item"))
            ...
            if item and item.get("type") in ("function_call", "tool_call"):
                ...
                buf[iid] = {
                    "name":   item.get("function", {}).get("name") or
                              item.get("tool_call", {}).get("name") or
                              item.get("name"),
                    "parts":  [],
        ...
        if "arguments.delta" in typ:
            ...
            if iid and iid in buf: buf[iid]["parts"].append(delta or "")
        ...
        if typ.endswith("arguments.done"):
            ...
            if iid and iid in buf:
                response_id = _finish_tool(
                    response_id, run_id,
                    buf[iid]["call_id"], buf[iid]["name"],
                    "".join(buf[iid]["parts"]), q
                ); buf.pop(iid, None)

        if typ in ("response.done", "response.completed", "response.output_text.done"):
            q.put(None); return response_id
...
def sse(q: queue.Queue[Any]) -> Generator[bytes, None, None]:
    ...
    while True:
        try:
            ...
            yield f"data: {tok}\n\n".encode(); keep_alive = time.time() + 20
...
def chat_stream():
    ...
    def work() -> None:
        stream = client.responses.create(
            model=MODEL, input=msg, previous_response_id=last_resp_id,
    ...
    return Response(stream_with_context(sse(q)),
...
def index():
    if request.method == "POST":
        ...
        if not f or not f.filename.endswith(".csv"):
            flash("Please upload a valid CSV."); return redirect(request.url)

        t0 = time.perf_counter()
        df = pd.read_csv(f); new_clients, invoices = {}, []
        for _, row in df.iterrows():
            ...
            invoices.append(Invoice(
                invoice_id=row["invoice_id"], amount=row["amount"],
                date_due=row["date_due"], status=row["status"], client=cl))
        db.session.add_all(new_clients.values()); db.session.add_all(invoices); db.session.commit()
        log.info("CSV: %d invoices, %d clients (%.3fs)", len(invoices), len(new_clients),
                 time.perf_counter() - t0)
        flash("Invoices uploaded successfully."); return redirect("/")
...
def export_invoice(invoice_id: int | None = None):
    ...
    csv_data = pd.DataFrame([{
        "client_name": r.client.name, "client_email": r.client.email,
        "invoice_id": r.invoice_id,   "amount": r.amount,
